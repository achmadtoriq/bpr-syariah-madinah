<main class="w-full mx-auto mt-14 md:mt-24 py-1">
    <div x-data="{ activeTab: '<?= $tabs[0]['id'] ?>' }" class="container mx-auto py-1 my-7 md:my-14 p-5">

        <div class="text-center mb-4 md:mb-14">
            <h1 class="text-xl md:text-3xl font-bold">Galeri Aktivitas Kami</h1>
            <!-- Bouncing dots -->
        ...
        <div class="justify-center flex">
            <div class="max-w-lg justify-center flex space-x-4 mb-2 md:mb-4 border-b-2 border-blue-500">
                <?php foreach ($tabs as $tab): ?>
                    <button @click="activeTab = '<?= $tab['id'] ?>'"
                        :class="activeTab === '<?= $tab['id'] ?>' ? 'bg-blue-500 font-semibold text-white rounded-t-xl' : 'text-gray-500'"
                        class="py-2 px-4 text-xs md:text-sm ">
                        <?= esc($tab['label']) ?>
                    </button>
                <?php endforeach; ?>
            </div>
        ...
        <div>
            <?php foreach ($tabs as $tab): ?>
                <div x-show="activeTab === '<?= $tab['id'] ?>'" class="transition duration-300">
                    <div class="mt-10 grid grid-cols-2 gap-x-3 gap-y-3 md:grid-cols-4">
                        <?php
